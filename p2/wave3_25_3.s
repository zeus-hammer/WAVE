...
getop:	mov 	ci,op
	and	$opMask, op
;;; swinnntch on the opcode to get type
	mov	TYPE(op), rip
...
oRHS:	mov 	$maskAddr, work0
	...
	cmovg	ADDR(work0), rip
;;; --------------------ADDRESSING MODES OF ARITHMETIC--------------------
;;; Immediate Mode
imd:	mov	ci, rhs
	and	$maskValue, rhs	;value
;;; 	cmove	INSTR(op),rip
	and	$maskExp, temp	;exponent
	shr	$9, temp
	shl	temp, rhs	;shifted value in rhs
	mov     INSTR(op), rip
...
;;; 	cmove	INSTR(op),rip	
	...
	and	$shopMask, temp	;temp now has the shop
	mov 	SHOP(temp), rip
...
rpm:	mov	ci, rhs	
	...
	mov	$maskLow4, temp
	and	ci, temp	;work0 now has src reg 3
	mov	REGS(temp), temp ;work0 now has whatever was stored in the correspondent register
	mul	temp, rhs
